...
Explanation:
	We have two queries:
		...
		One we hit n=10, we print the first ten tearms as a single line of space-separated integers.
...
public class Solution {
    ...
    private static int squaredNumber(int number, int power) {
        if (power == 0) { return 1;}
        int x = number;
        while (power-- > 1) {
            x *= number;
